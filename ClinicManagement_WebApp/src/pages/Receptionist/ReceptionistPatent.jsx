...
import instanceReceptionist from "../../axiosReceptionist";
...
const ReceptionistPatent = () => {
    ...
    const checkTimeSlotAvailability = async (time, date, roomId = null, staffId = null) => {
        try {
            const response = await instanceReceptionist.get('/api/receptionist/appointments/count-by-timeslot', {
                params: {
                    time: time,
                    date: date,
                    room_id: roomId,
    ...
    const api = {
        getRooms: async () => {
            try {
                const response = await instanceReceptionist.get('/api/receptionist/rooms');
        ...
        getDoctorsWithSchedules: async (date, roomId = null) => {
            try {
                ...
                const response = await instanceReceptionist.get('/api/receptionist/medical-staff/schedules', { params });
                return response.data;
            } catch (error) {
        ...
        getAllPatients: async () => {
            try {
                const response = await instanceReceptionist.get('/api/receptionist/patients');
        ...
        createPatient: async (patientData) => {
            try {
                const response = await instanceReceptionist.post('/api/receptionist/patients', patientData);
                return response.data;
            } catch (error) {
        ...
        completeReception: async (receptionData) => {
            try {
                const response = await instanceReceptionist.post('/api/receptionist/complete', receptionData);
                return response;
            } catch (error) {
    ...
    const handlePatientSelect = async (selectedOption, action) => {
        ...
        if (selectedOption?.isCreateNew) {
            ...
            try {
                ...
                const response = await instanceReceptionist.get(`/api/receptionist/patients/${selectedOption.value}`);
    ...
    const handleCreateAll = async () => {
        ...
        try {
            ...
            if (result && result.success === true) {
                showToastMessage('success', `Đã tiếp nhận thành công! Số thứ tự: ${result.data.queueNumber || 'N/A'}`);
