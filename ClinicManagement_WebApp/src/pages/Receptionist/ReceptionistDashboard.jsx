...
const ReceptionistDashboard = () => {
    ...
    const fetchAllQueues = async () => {
        setLoading(true);
        try {
            const queueResponse = await instanceReceptionist.get(`/api/receptionist/queue`);
    ...
    const fetchQueueByRoom = async (roomId) => {
        setLoading(true);
        try {
            const response = await instanceReceptionist.get(
    ...
    const getRoomColor = (roomId) => {
        ...
        if (isNaN(id) || id < 0) {
            return "secondary"; // fallback không bị trắng
        }
    ...
    const isRoomOccupied = (roomId) => {
        return queue.some(
            (item) => item.room === roomId && item.Status === "InProgress"
        );
    };
    // Handle call patient
    const handleCallPatient = async (queueId) => {
        ...
        try {
            await instanceReceptionist.put(`/api/receptionist/queue/${queueId}/status`, { Status: "InProgress" });

            // Thành công -> Update UI
            setQueue(prev => prev.map(item => item.QueueId === queueId ? { ...item, Status: "InProgress" } : item));
    ...
    const confirmCancelAppointment = async () => {
        if (deleteModal.patient) {
            try {
                await instanceReceptionist.put(
    ...
    const priorityCount = filteredQueue.filter(
        (item) => item.QueuePosition === 1 && item.Status === "Waiting"
    ).length;
                                ...
                                <tbody>
                                    {currentItems.length > 0 ? (
                                        currentItems.map((item, index) => (
                                            <tr
                                                key={item.QueueId}
                                                className={`table-row-hover ${item.Status === "InProgress" ? "table-active" : ""
                                                    } ${item.QueuePosition === 1 && item.Status === "Waiting"
