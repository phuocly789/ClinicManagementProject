...
const RevenueForecast = ({ forecastData }) => {
    ...
    const chartData = [
        ...
        {
            name: 'DỰ BÁO',
            revenue: forecastData.forecast.predictedRevenue,
            type: 'forecast',
            fullLabel: `Dự báo tháng ${forecastData.forecast.nextMonth}/${forecastData.forecast.nextYear}`
        }
    ];
                        ...
                        <div className="col-6">
                            ...
                            <strong className="text-success">
                                {new Intl.NumberFormat('vi-VN', {
                                    style: 'currency',
                                    currency: 'VND'
                                }).format(forecastData.forecast.predictedRevenue)}
                            </strong>
                        </div>
                        <div className="col-6">
                            <small className="text-muted d-block">Độ chính xác</small>
                            <strong className="text-warning">
                                {forecastData.forecast.confidence * 100}%
                            </strong>
...
const PrescriptionAnalytics = ({ analyticsData }) => {
    ...
    const medicineChartData = topMedicines.slice(0, 5).map(med => ({
        ...
        quantity: med.totalQuantity || med.total_quantity,
                        ...
                        <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                            {topDoctors.slice(0, 5).map((doctor, index) => (
                                <div key={doctor.StaffId} className="d-flex align-items-center justify-content-between p-2 border-bottom">
                                    ...
                                    <span className="badge bg-light text-dark">{doctor.prescriptionCount || doctor.prescription_count} đơn</span>
                                </div>
                            ))}
                        </div>
                    </div>
                                    ...
                                    {bestSellingMedicines.slice(0, 5).map((medicine, index) => (
                                        <tr key={medicine.medicineId}>
                                            ...
                                            <td className="text-end fw-bold text-primary">
                                                {new Intl.NumberFormat('vi-VN').format(medicine.totalSold || medicine.total_sold)}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
...
const AdminDashboard = () => {
    ...
    const fetchData = useCallback(async (start, end) => {
        setLoading(true);
        try {
            const [lowStockRes, combinedRes, forecastRes, analyticsRes] = await Promise.all([
                instance.get('Medicine/Inventory-Warnings?threshold=300'),
                instance.get(`Reports/CombinedRevenue?startDate=${start}&endDate=${end}`),
                instance.get('Reports/RevenueForecast'),
                instance.get('Reports/PrescriptionAnalytics')
            ]);

            // XỬ LÝ DỮ LIỆU TỪ COMBINED API
            if (combinedRes && combinedRes.content) {
                const data = combinedRes.content;

                // Set data cho HistoricalStats
                setStats({
                    totalAppointmentsToday: data.totalAppointmentsToday,
                    completedAppointmentsToday: data.completedAppointmentsToday,
                    pendingInvoicesCount: data.pendingInvoicesCount,
                });

                // Set data cho TotalRevenue
                setTotalRevenue({
            ...
            // Xử lý các API khác
            setLowStockMedicines(lowStockRes.content || lowStockRes || []);
            setRevenueForecast(forecastRes.content || forecastRes);
            setPrescriptionAnalytics(
                analyticsRes.content ?? {
                    topMedicines: [],
                    topDoctors: [],
                    bestSellingMedicines: []
                }
